...
static struct platform_device_id msix_qdma_id_table[] = {
	{ XOCL_DEVNAME(XOCL_QDMA_MSIX), (kernel_ulong_t)&msix_qdma_priv },
	{ },
};

static struct platform_driver	mgmt_msix_qdma_driver = {
...
{
	return platform_driver_register(&mgmt_msix_qdma_driver);
}

void xocl_fini_mgmt_msix_qdma(void)
{
	return platform_driver_unregister(&mgmt_msix_qdma_driver);
}
